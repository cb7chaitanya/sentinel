...
from vision.core.di import get_detection_pipeline_runner


@asynccontextmanager
async def lifespan(app: FastAPI) -> AsyncGenerator[None]:
    runner = get_detection_pipeline_runner()
    await runner.start()
    try:
        yield
    finally:
...
def create_app() -> FastAPI:
    ...
    app = FastAPI(title="Sentinel Vision Service", version="0.1.0", lifespan=lifespan)
